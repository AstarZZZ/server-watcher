...
- 登录页可填写 SSH 主机、端口、用户名和密码，适合直接在个人电脑本地运行后连接远程服务器。
- 登录密码只用于当次 SSH 认证，不写入数据库或日志。
- 远程文件页提供类似 Finder 的目录浏览、搜索、大小排序和空间分支视图；用户点击查询后由远程 `du` 命令统计，并明确显示等待状态。
- 已查询目录会在当前登录会话中缓存 15 分钟，返回目录时直接复用；只有点击刷新按钮才会强制重新查询。
- 可多选文件或文件夹并通过 SSH 永久删除。删除前必须重新输入密码并键入“永久删除”，且范围被限制在当前账户主目录。
...
browser
  ...
  -> Node.js backend on :33099
       ...
       -> SSH server for auth, remote files, terminal, kill, sudo systemctl
...
- SSH 主机：服务器 IP 或域名。
- 端口：通常为 `22`，也可以使用服务器自定义端口。
- 用户名和密码：你自己的服务器账户。

登录后进入“远程文件”，即可从该账户的主目录开始浏览。主机、端口和用户名会保存在浏览器本地以便下次填写；SSH 密码只存在于当前页面内存，刷新、断开或退出后需要重新输入。
...
1. 浏览器提交 SSH 主机、端口、Linux 用户名和 SSH 密码。
2. 后端尝试连接指定 SSH 服务器。
...
- 只允许浏览和删除远程账户真实主目录内的路径，不能删除主目录本身。
- 目录列表和大小统计使用远程 `find`、`stat`、`du`、`readlink` 和 `bash`。
- 删除使用远程 `rm -rf -- <路径>`，权限完全遵循该 SSH 账户；工具不会使用 sudo。
- 删除不会进入废纸篓，确认后不可恢复。建议先核对路径和预计释放空间。
- 单次最多删除 100 个项目；超大目录的统计可能需要一些时间。
- 首次进入目录需要点击查询并等待；查询结果缓存 15 分钟，删除成功后相关缓存会立即清空。
